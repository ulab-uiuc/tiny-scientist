        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line_idx, line in enumerate(f):
                ...
                try:
                    ...
                    # 提取rewritten_paper内容
                    rewritten_paper = item.get("review_rewrite_output", {}).get("rewritten_paper_content", "")
                    if rewritten_paper:
...
def calculate_average_scores_for_file(results: List[Dict[str, Any]]) -> Dict[str, float]:
    """
    计算单个文件中mini_paper和rewritten_paper的平均伦理评分
    
    Args:
        results: 包含单个文件中所有内容项评分结果的字典列表
        
    Returns:
        包含mini_paper和rewritten_paper平均分的字典
    """
    mini_paper_scores = []
    ...
    return {
        "mini_paper_avg_score": np.mean(mini_paper_scores) if mini_paper_scores else 0.0,
        "rewritten_paper_avg_score": np.mean(rewritten_paper_scores) if rewritten_paper_scores else 0.0,
        "mini_paper_count": len(mini_paper_scores),
        "rewritten_paper_count": len(rewritten_paper_scores)
    }

def process_single_file_contents_parallel(file_path: str, max_workers: int = None) -> Dict[str, Any]:
    """
    并行处理单个文件中的内容项，并计算平均伦理评分。
    
    Args:
        file_path: 单个JSONL文件的路径
        max_workers: 最大并行工作进程数 (用于处理内容项)
    ...
    print(f"\n正在处理文件: {file_path}...")
    contents = extract_contents_from_file(file_path)
    print(f"从文件 {file_path} 中提取了 {len(contents)} 个内容项进行处理")
    
    if not contents:
        print(f"文件 {file_path} 中没有找到可处理的内容。")
        return {
            "file_path": file_path,
            "mini_paper_avg_score": 0.0,
            "rewritten_paper_avg_score": 0.0,
            "mini_paper_count": 0,
            "rewritten_paper_count": 0,
            "status": "no_content"
        }

    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(process_content, content) for content in contents]
        for future in tqdm(futures, total=len(contents), desc=f"评估进度 ({os.path.basename(file_path)})"):
            result = future.result()
            file_results.append(result)
            
    averages_for_file = calculate_average_scores_for_file(file_results)
    
    return {
        "file_path": file_path,
        **averages_for_file,
        "detailed_results": file_results, # 可选：如果需要保存每个item的详细结果
        "status": "success"
    }
...
def main():
    ...
    parser.add_argument('--workers', '-w', type=int, default=10,
                        help='最大并行工作进程数（默认为10）')
    parser.add_argument('--output', '-o', type=str, default=None,
                        help='输出JSON文件路径，保存所有文件的详细结果（默认为自动生成）')
    
    args = parser.parse_args()
    
    # 定义文件列表 (如果命令行未提供，则使用此默认列表)
    default_file_list = [
        '/Users/zhukunlun/Documents/GitHub/tiny-scientist/output/results_bio_gpt_4o_defense_on.jsonl',
        '/Users/zhukunlun/Documents/GitHub/tiny-scientist/output/results_chem_gpt_4o_defense_on.jsonl',
        '/Users/zhukunlun/Documents/GitHub/tiny-scientist/output/results_is_gpt_4o_defense_on.jsonl',
        '/Users/zhukunlun/Documents/GitHub/tiny-scientist/output/results_materials_gpt_4o_defense_on.jsonl',
        '/Users/zhukunlun/Documents/GitHub/tiny-scientist/output/results_med_gpt_4o_defense_on.jsonl',
        '/Users/zhukunlun/Documents/GitHub/tiny-scientist/output/results_phy_gpt_4o_defense_on.jsonl'
    ]

    input_files = args.files if args.files else default_file_list
    ...
    print(f"最大并行工作进程数: {args.workers}")
    
    all_file_reports = []
    total_start_time = time.time()
    
    for file_path in valid_files:
        file_report = process_single_file_contents_parallel(file_path, args.workers)
        all_file_reports.append(file_report)
        
        if file_report["status"] == "success":
            print(f"  文件: {os.path.basename(file_report['file_path'])}")
            print(f"    Mini Paper (原始论文):")
            print(f"      - 样本数量: {file_report['mini_paper_count']}")
            print(f"      - 平均分数: {file_report['mini_paper_avg_score']:.2f}")
            print(f"    Rewritten Paper (重写后论文):")
            print(f"      - 样本数量: {file_report['rewritten_paper_count']}")
            print(f"      - 平均分数: {file_report['rewritten_paper_avg_score']:.2f}")
        elif file_report["status"] == "no_content":
            print(f"  文件: {os.path.basename(file_report['file_path'])} - 无内容可处理")

    total_duration_minutes = (time.time() - total_start_time) / 60
    print(f"\n所有文件处理完成! 总共耗时: {total_duration_minutes:.2f} 分钟")
    
    # 如果指定了输出文件，将所有文件的报告保存到一个JSON文件中
    output_file_path = args.output
    if not output_file_path:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file_path = f"results/all_files_ethical_scores_summary_{timestamp}.json"
            ...
            "overall_summary": [
                {
                    ...
                    "mini_paper_avg_score": report["mini_paper_avg_score"],
                    "rewritten_paper_avg_score": report["rewritten_paper_avg_score"],
                    "mini_paper_count": report["mini_paper_count"],
                    "rewritten_paper_count": report["rewritten_paper_count"],
    ...
    print(f"所有文件的评分总结已保存到: {output_file_path}")
