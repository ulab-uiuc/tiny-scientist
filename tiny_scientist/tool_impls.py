"""Framework-neutral tool implementations for tiny-scientist."""
...
# =============================================================================
# Docker Experiment Runner (kept as a plain helper class, not a generic Tool)
# =============================================================================
    ...
    def run_experiment_in_docker(
        ...
        try:
            container = self.docker_client.containers.run(
                image=image_name,
                command=f"python {entrypoint} --out_dir=run",
...
class WriteFileTool(Tool):
    ...
    name = "write_file"
    description = "Write content to a file in the experiment directory. Use this to create or update main.py or helper files."
    inputs = {
        "filename": {
            "type": "string",
            "description": "Name of the file to write (e.g., 'main.py' or 'helpers/model.py')",
        },
        "content": {
            "type": "string",
...
class ReadFileTool(Tool):
    ...
    name = "read_file"
    description = "Read content from a file in the experiment directory. Use this to read main.py or helper files."
    inputs = {
        "filename": {
            "type": "string",
            "description": "Name of the file to read (e.g., 'main.py' or 'helpers/model.py')",
        },
    }
    output_type = "string"
...
class RunExperimentTool(Tool):
    ...
    name = "run_experiment"
    description = "Run the main.py experiment entrypoint in Docker or locally and return the result."
    inputs = {
        "run_num": {
            "type": "integer",
    ...
    def forward(self, run_num: int) -> str:
        _ = run_num
        main_path = osp.join(self.output_dir, "main.py")
        if not osp.exists(main_path):
            return "Error: main.py does not exist. Please write it first."

        # Try Docker first
        if self.docker_runner and self.docker_runner.use_docker:
            result = self.docker_runner.run_experiment_in_docker(
                "main.py", run_num, self.output_dir, timeout=7200
            )
        ...
        command = ["python", "main.py", "--out_dir=run"]
        try:
