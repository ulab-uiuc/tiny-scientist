...
class BaseFormat(abc.ABC):
    @abc.abstractmethod
    def run(self, template: str) -> Dict[str, Dict[str, str]]:
        pass
    ...
    def _assemble_body(self, contents: Dict[str, Dict]) -> str:
        section_order = [
            "Abstract",
            "Introduction",
            "Background",
...
class ICLRFormat(BaseFormat):
    ...
    def run(self, 
        ...
        self._compile_latex(dest_template_dir, self.template, output_pdf_path, timeout)
        self.watermark._add_watermark(                            
                            output_pdf_path,
                            watermark_text="CAUTION!!! THIS PAPER WAS AUTONOMOUSLY GENERATED BY THE TINY_SCIENTIST",
                            output_pdf_path=output_pdf_path
