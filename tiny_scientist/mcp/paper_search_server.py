    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if hasattr(e, "response"):
                print(
                    f"[Paper Search] Response text: {e.response.text if e.response else 'No response'}"
                )
            return None


async def make_openalex_request(
    query: str, result_limit: int
...
def extract_openalex_work_info(
    ...
    venue = next(
        (loc["source"]["display_name"] for loc in work["locations"] if loc["source"]),
        "Unknown",
    )
...
async def search_papers(query: str, result_limit: int = 3) -> str:
    ...
    for paper in papers:
        ...
        if bibtex and bibtex != "N/A":
            ...
            results[title] = {"title": title, "bibtex": bibtex}
...
async def get_paper_details(paper_id: str) -> str:
    ...
    url = f"{S2_API_BASE}/paper/{paper_id}"
    params = {
        "fields": "title,authors,venue,year,abstract,citationCount,citationStyles"
    }
