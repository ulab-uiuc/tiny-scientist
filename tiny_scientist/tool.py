...
class DockerExperimentRunner:
    """Handles Docker-based experiment execution with dynamic dependency management."""
    
    def __init__(self, docker_image: str = "tiny-scientist-ml", docker_base: str = "python:3.11-slim"):
    ...
    def detect_required_packages(pyfile: str, base_packages: Optional[Set[str]] = None) -> List[str]:
        ...
        if base_packages is None:
            base_packages = set([
                'numpy', 'pandas', 'scikit-learn', 'matplotlib', 'seaborn', 'torch', 'tensorflow',
                'transformers', 'datasets', 'evaluate', 'wandb', 'tqdm', 'requests', 'pillow'
            ])
        ...
        stdlib_modules = {
            'os', 'sys', 'json', 'time', 'datetime', 'random', 'math', 'statistics',
            'collections', 'itertools', 'functools', 'pathlib', 'argparse', 'subprocess',
            'tempfile', 'shutil', 'glob', 're', 'urllib', 'http', 'socket', 'threading',
            'multiprocessing', 'asyncio', 'logging', 'pickle', 'copy', 'typing',
            'abc', 'enum', 'dataclasses', 'contextlib', 'weakref', 'gc', 'inspect',
            'traceback', 'pdb', 'unittest', 'doctest', 'hashlib', 'hmac', 'base64',
            'zlib', 'gzip', 'bz2', 'lzma', 'zipfile', 'tarfile', 'csv', 'configparser',
            'xml', 'html', 'email', 'smtplib', 'ftplib', 'telnetlib', 'poplib',
            'imaplib', 'nntplib', 'socketserver', 'xmlrpc', 'webbrowser', 'cgi',
            'cgitb', 'wsgiref', 'urllib3', 'ssl', 'select', 'signal', 'pwd', 'grp',
            'pwd', 'spwd', 'crypt', 'termios', 'tty', 'pty', 'fcntl', 'pipes',
            'posix', 'nt', 'msvcrt', 'winreg', 'winsound', 'msilib', 'win32com',
            'win32api', 'win32gui', 'win32con', 'win32file', 'win32pipe', 'win32event',
            'win32process', 'win32security', 'win32service', 'win32serviceutil',
            'win32timezone', 'pythoncom', 'pywintypes', 'win32ui', 'win32print',
            'win32clipboard', 'win32api', 'win32gui', 'win32con', 'win32file',
            'win32pipe', 'win32event', 'win32process', 'win32security', 'win32service',
            'win32serviceutil', 'win32timezone', 'pythoncom', 'pywintypes', 'win32ui',
            'win32print', 'win32clipboard'
        }
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                try:
                    ...
                    print(f"[Docker] Container {container.id[:12]} finished with status {result['StatusCode']}")
                ...
                if result['StatusCode'] == 0:
                    ...
                    if "ModuleNotFoundError" in logs:
                        ...
                        try:
                            ...
                            retry_image_name = f"tiny-scientist-retry-{hash(missing_pkg)}"
                            with tempfile.TemporaryDirectory() as retry_tmpdir:
                                # Write requirements.txt with the missing package
                                with open(os.path.join(retry_tmpdir, "requirements.txt"), "w") as f:
                                    f.write(missing_pkg + "\n")
...
"""
                                with open(os.path.join(retry_tmpdir, "Dockerfile"), "w") as f:
                                    f.write(retry_dockerfile)
                                
                                # Build retry image
                                self.docker_client.images.build(path=retry_tmpdir, tag=retry_image_name, rm=True)
                                ...
                                print(f"[Docker] Retry container {retry_container.id[:12]} started")
                                retry_result = retry_container.wait(timeout=timeout)
                                retry_logs = retry_container.logs().decode('utf-8')
                                ...
                                if retry_result['StatusCode'] == 0:
                                    ...
                                    return retry_result['StatusCode'], retry_logs
